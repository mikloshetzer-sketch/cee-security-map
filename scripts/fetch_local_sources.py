...
# Build taxonomy-derived article candidate terms only after the text helpers
# have been defined. This prevents module-import NameError failures.
...
def ensure_classifier_fields(features):
    """Guarantee that every emitted feature is classifier-backed.

    The normal path already writes these fields in ``build_feature``. This
    defensive final gate prevents a legacy/raw feature from reaching
    ``local_events.geojson`` if a future helper or merge path introduces one.
    Records that cannot be validated as an actual taxonomy incident are
    dropped rather than emitted in a partially classified form.
    """
    clean = []
    repaired = 0
    dropped = 0
    ...
    for feature in features:
        ...
        missing = [key for key in REQUIRED_CLASSIFIER_FIELDS if key not in props]
        ...
        incident = CLASSIFIER.classify(
            ...
            source=str(props.get("source") or "local_media"),
            url=str(props.get("url") or ""),
            published=props.get("time") or props.get("published"),
            country=props.get("country"),
            city=props.get("city") or (props.get("place") if props.get("geocode_quality") == "city" else None),
            latitude=lat,
            longitude=lon,
            source_is_official=False,
            source_count=max(1, int(props.get("source_count") or 1)),
            extra_context={
                "pipeline": "local_events_final_gate",
                "repair_missing_fields": missing,
            },
        )

        if not incident.matched_rule_id or not incident.actual_incident:
            dropped += 1
            print(
                "[local-final-gate] dropped unvalidated feature:",
                props.get("title"),
                "missing=", missing,
            )
            continue

        classifier_props = incident.to_geojson_properties()
        props.update(classifier_props)
        props.update({
            "incident_family": incident.family,
            "incident_subcategory": incident.subcategory,
            "incident_subtype": incident.subtype,
    ...
    print(
        f"[local-final-gate] accepted={len(clean)} repaired={repaired} dropped={dropped}"
    )
    return clean
...
def main():
    ...
    features = ensure_classifier_fields(features)
